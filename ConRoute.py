...
def LS(ip):
    ...
    for route in graph.keys():
        Routing_Table[route]={"Next_Node":"0","Distance":99999}
    ...
    while N :
        ...
        for neihbour in graph[next_route].keys():
            ...
            D[neihbour]=D[next_route]+graph[next_route][neihbour] if D[next_route]+graph[next_route][neihbour]<D[neihbour] else D[neihbour]
...
def ask(coon,addr, dip): #TODO
    if dip in ALL_Routing_table[addr]:
        via=ALL_Routing_table[addr][dip]["Next_Node"]
        dis=ALL_Routing_table[addr][dip]["Distance"]
    else:
        via="None"
        dis=99999
